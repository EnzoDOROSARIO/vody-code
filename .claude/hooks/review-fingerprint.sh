...
# which paths differ from HEAD, and what each of those now holds.
#
...
# sit on is not part of that work. Hashing the commit too retired a good record every
# time history moved under it — an amend, a rebase, a `reset --soft` — none of which
# change a line of what a reviewer read.
#
